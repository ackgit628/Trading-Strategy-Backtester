...
# --- Phase 3: Signal Generation (with Stop-Loss) ---
...
# 2. Define trading and stop-loss thresholds
upper_threshold = 1.5
lower_threshold = -1.5
...
# 3. Create signals
data['signal'] = 0
data.loc[data['z_score'] > upper_threshold, 'signal'] = -1  # Short signal
data.loc[data['z_score'] < lower_threshold, 'signal'] = 1   # Long signal

# 4. Create position with exits
data['position'] = data['signal'].replace(0, method='ffill')
# Take-Profit Exit: z-score crosses the mean
data.loc[(data['position'] == 1) & (data['z_score'] > 0), 'position'] = 0
data.loc[(data['position'] == -1) & (data['z_score'] < 0), 'position'] = 0
# Stop-Loss Exit: z-score hits the stop-loss threshold
data.loc[(data['position'] == 1) & (data['z_score'] < -stop_loss_threshold), 'position'] = 0
data.loc[(data['position'] == -1) & (data['z_score'] > stop_loss_threshold), 'position'] = 0
...
print("\n--- Data with Z-Score, Positions, and Stop-Loss ---")
print(data.tail(10))

# Plot the Z-score and thresholds
data['z_score'].plot(figsize=(14, 7), title='Spread Z-Score with Stop-Loss')
...
plt.axhline(stop_loss_threshold, color='orange', linestyle=':', label='Stop-Loss')
plt.axhline(-stop_loss_threshold, color='orange', linestyle=':', label='Stop-Loss')
